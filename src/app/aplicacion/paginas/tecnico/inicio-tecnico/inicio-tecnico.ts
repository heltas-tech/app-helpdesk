// src/app/aplicacion/tecnico/inicio-tecnico/inicio-tecnico.component.ts
...
export class InicioTecnicoComponent implements OnInit, OnDestroy {
  ...
  private loadTecnicoInfo() {
    ...
    this.tecnicoInfo = {
      nombre: usuario?.nombre_usuario || 'Técnico',
  ...
  private loadDashboardStats() {
    ...
    this.dashboardService.getDashboardTecnico().subscribe({
      next: (response) => {
        if (response.isSuccess && response.data) {
          this.stats = response.data;
          this.updateChartsWithStats();
        } else {
          this.loadStatsManual();
        }
  ...
  private async loadStatsManual() {
    try {
      const ticketsResponse = await this.ticketsService.lista().toPromise();
      if (ticketsResponse?.isSuccess && ticketsResponse.data) {
        const ticketsTecnico = Array.isArray(ticketsResponse.data) 
          ? ticketsResponse.data.filter((t: any) => t.estado && t.tecnico_id)
          : [];
        this.calcularStatsManual(ticketsTecnico);
        this.cargarTicketsRecientesManual(ticketsTecnico);
        this.actualizarGraficosManual();
      }
    } catch (error) {
      console.error('Error cargando tickets manualmente:', error);
    }
  }
  ...
  private calcularStatsManual(tickets: any[]) {
    ...
    this.stats = {
      ...
      ticketsAsignados: tickets.length,
      ticketsAbiertos: tickets.filter(t => !t.fecha_resolucion).length,
      ticketsEnProceso: tickets.filter(t => !t.fecha_resolucion && t.estado_ticket === 'EN_PROCESO').length,
      ticketsResueltos: ticketsResueltos.length,
      ticketsUrgentes: tickets.filter(t => t.prioridad?.nivel >= 3).length,
      ...
      ticketsResueltosHoy: ticketsResueltos.filter(t => new Date(t.fecha_resolucion) >= inicioDia).length,
      tiempoPromedioResolucion: this.calcularTiempoPromedioManual(ticketsResueltos),
      satisfaccionPromedio: this.calcularSatisfaccionManual(ticketsResueltos),
      slaCumplido: this.calcularSLAManual(ticketsResueltos),
      eficienciaGeneral: tickets.length > 0 ? Math.round((ticketsResueltos.length / tickets.length) * 100) : 0
    };
  }

  private calcularSLAManual(ticketsResueltos: any[]): number {
    ...
    ticketsResueltos.forEach(ticket => {
      ...
      const tiempoSLA = ticket.prioridad?.tiempo_resolucion || 72;
  ...
  private calcularTicketsSLAVencido(tickets: any[]): number {
    ...
    return tickets.filter(ticket => {
      if (ticket.fecha_resolucion) return false;
      const creado = new Date(ticket.fecha_creacion);
      const tiempoSLA = ticket.prioridad?.tiempo_resolucion || 72;
  ...
  private calcularTiempoPromedioManual(ticketsResueltos: any[]): string {
  ...
  private calcularSatisfaccionManual(ticketsResueltos: any[]): number {
    if (ticketsResueltos.length === 0) return 0;
  ...
  private verificarSLAEstado(ticket: any): string {
    if (ticket.fecha_resolucion) return EstadoSLA.DENTRO_TIEMPO;
    if (!ticket.prioridad?.tiempo_resolucion) return EstadoSLA.SIN_SLA;
  ...
  private actualizarGraficosManual() {
    ...
    const resueltos = tickets.filter(t => t.fecha_resolucion).length;
    const enProceso = tickets.filter(t => !t.fecha_resolucion && t.estado === 'EN_PROCESO').length;
    const abiertos = tickets.filter(t => !t.fecha_resolucion && t.estado !== 'EN_PROCESO').length;
    const urgentes = tickets.filter(t => t.prioridad_nivel && t.prioridad_nivel >= 3).length;

    this.distributionChartOptions.series = [resueltos, enProceso, abiertos, urgentes];
    this.efficiencyChartOptions.series = [this.stats.eficienciaGeneral ?? 0];
  }
      ...
      next: (response) => {
        if (response.isSuccess && response.data) {
          this.ticketsSLAVencidos = response.data
            .filter(ticket => ticket.sla_estado === EstadoSLA.VENCIDO)
            .slice(0, 3)
            .map(ticket => ({ ...ticket, ticketOriginal: ticket }));
          
          this.ticketsSLAPorVencer = response.data
            .filter(ticket => ticket.sla_estado === EstadoSLA.POR_VENCER)
            .slice(0, 3)
            .map(ticket => ({ ...ticket, ticketOriginal: ticket }));
        }
      },
  ...
  private updateChartsWithStats() {
    this.distributionChartOptions.series = [
      ...
      Math.max(0, (this.stats.ticketsAsignados || 0) - this.stats.ticketsResueltos - (this.stats.ticketsEnProceso || 0)),
  ...
  private getDistributionChartOptions(): ApexOptions {
    return {
      ...
      labels: ['Resueltos', 'En Proceso', 'Pendientes', 'Urgentes'],
